...
function renderRank(){
  ...
  $('#rank').innerHTML = st.map((c,i)=>{
    ...
    // 排行口径与「日度实际 GMV」卡片保持一致：主数字展示 actual；达成率仍按 actual_forecast/target
    const amt=na?(wan(c.actual)+'万 / 无目标'):(wan(c.actual)+'万 / '+wan(c.op)+'万');
    return `<div class="rrow">
      ...
      <div class="num"><div class="rt tnum" style="color:${color||'#9a9183'}">${na?'—':c.rate+'%'}</div>
        ...
        ${!na?`<div class="amt2 tnum" style="color:#9a9183;font-size:11px">实际+预估 ${wan(c.af)}万</div>`:''}
