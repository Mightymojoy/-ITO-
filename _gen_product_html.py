...
function showSKUDetail(skuKey){
  ...
  new Chart(document.getElementById('skuDetailTrend'),{type:'line',data:{labels:dates,datasets:[{label:'今年',data:trendNow,borderColor:'#2563eb',backgroundColor:'rgba(37,99,235,0.1)',fill:true,tension:.3,pointRadius:1,pointHitRadius:20,borderWidth:2},{label:'去年同期',data:showYoy?yoyTrend:[],borderColor:'#9ca3af',backgroundColor:'transparent',fill:false,tension:.3,pointRadius:1,pointHitRadius:20,borderWidth:2,borderDash:[5,5]}].filter(function(ds){return ds.data.length>0})},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{enabled:true,mode:'index',intersect:false,callbacks:{title:function(items){return items[0].label},label:function(ctx){var p=Math.round(ctx.parsed.y);return ctx.dataset.label+': '+(metric==='amt'?'¥'+p.toLocaleString()+'元':p.toLocaleString()+'件')}}}},hover:{mode:'index',intersect:false},scales:{y:{beginAtZero:true,ticks:{callback:function(v){return Math.round(metric==='amt'?v/10000:v)+''}}}}}});
}
